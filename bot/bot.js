...
const ABOUT_POLYGON = "Polygon is a protocol and a framework for building and connecting Ethereum-compatible blockchain networks";
...
const URL_ALL_CONTRACTS_JSON = "https://static.matic.network/network/testnet/mumbai/index.json";
...
const URL_SOCIAL_YOUTUBE = "https://www.youtube.com/channel/UCvjEq17wWLHFVYwU3OOXPtQ";
...
const URL_ROADWEB3 = "https://vg8il995a1n6thi7133ev8s91e3pmmvnulh16b3ev3tokmtsmk2ikoo.siasky.net/";
...
const URL_API_PRICE = "https://api-testnet.polygonscan.com/api?module=stats&action=maticprice&apikey=";
const URL_API_BALANCE = "https://api-testnet.polygonscan.com/api?module=account&action=balance&address=";
const URL_API_NFT = "https://api-testnet.polygonscan.com/api?module=account&action=balance&address=0xA73721242EeAb1B96B79fd6Cdd5ac7182Ad21cEe&apikey=";
    ...
    try {
        if (msg.content.startsWith("!mybalance")) {
            ...
            const balanceEmbed = new discord_js_1.MessageEmbed()
                ...
                .addField("Address:", `'0xA73721242EeAb1B96B79fd6Cdd5ac7182Ad21cEe'`, true)
                .addField("Current balance", `${accountBalance / 10n ** TOKEN_DECIMAL} ${TOKEN_NAME}`, true)
        ...
        if (msg.content.startsWith("!balance")) {
            ...
            const url = URL_API_BALANCE + queryaddress + "&apikey=" + process.env.API_KEY;
            const data = await fetch_json_1.fetchJson.get(url);
            let accountBalance = BigInt(data.result);
            const balanceEmbed = new discord_js_1.MessageEmbed()
                .setColor(EMBED_COLOR_SECONDARY)
        ...
        if (msg.content === "!mynft") {
            ...
            const data = await fetch_json_1.fetchJson.get(url);
            const rewardEmbed = new discord_js_1.MessageEmbed()
                .setColor(EMBED_COLOR_SECONDARY)
                .setDescription(BOT_NAME)
                .setURL(URL_DISCORD)
                .setAuthor("Author: " + AUTHOR, POLYGON_LOGO_COLOR_REVERSE, URL_HELP)
        ...
        if (msg.content === "!transaction") {
            const url = "https://api-testnet.polygonscan.com/api?module=account&action=txlist&address=0xA73721242EeAb1B96B79fd6Cdd5ac7182Ad21cEe&startblock=0&endblock=99999999&page=1&offset=1&sort=asc&apikey=" +
                process.env.API_KEY;
            const data = await fetch_json_1.fetchJson.get(url);
        ...
        if (msg.content === "!price") {
            ...
            const data = await fetch_json_1.fetchJson.get(url);
            const priceEmbed = new discord_js_1.MessageEmbed()
                .setColor(EMBED_COLOR_SECONDARY)
                .setDescription(BOT_NAME)
                .setURL(URL_DISCORD)
                .setAuthor("Author: " + AUTHOR, POLYGON_LOGO_COLOR_REVERSE, URL_HELP)
                .setThumbnail(IMG_MARKET)
                .addField(`Matic BTC ${data.result.maticbtc}`, " timestamp: " + `${data.result.maticbtc_timestamp}`, false)
                .addField(`Matic USD ${data.result.maticusd}`, " timestamp: " + `${data.result.maticusd_timestamp}`, false)
                .setImage(IMG_POLYGONSCAN)
                .setFooter(BOT_NAME_FOOTER, POLYGON_GLYPH_COLOR_REVERSE)
                .setTimestamp();
        ...
        if (msg.content === "!help") {
            const socialEmbed = new discord_js_1.MessageEmbed()
                ...
                .setURL(URL_DISCORD)
                .setAuthor("author: " + msg.author.username, POLYGON_LOGO_COLOR, URL_POLYGON)
                .setDescription(BOT_NAME)
        ...
        if (msg.content === "!social") {
            const socialEmbed = new discord_js_1.MessageEmbed()
                ...
                .setDescription(BOT_NAME)
                .setThumbnail(POLYGON_LOGO)
                .addFields({ name: "github", value: URL_SOCIAL_GITHUB, inline: true }, { name: "twitter", value: URL_SOCIAL_TWITTER, inline: true }, { name: "chat", value: URL_SOCIAL_CHAT, inline: true }, { name: "youtube", value: URL_SOCIAL_YOUTUBE, inline: true }, { name: "redit", value: URL_SOCIAL_REDIT, inline: true }, { name: "telegram", value: URL_SOCIAL_TELEGRAM, inline: true })
                .setTimestamp()
                .setFooter(BOT_NAME_FOOTER, POLYGON_GLYPH_COLOR_REVERSE);
            msg.channel.send(socialEmbed);
        }
        if (msg.content === "!web3") {
            const web3Embed = new discord_js_1.MessageEmbed()
                .setColor(EMBED_COLOR_PRIMARY)
                .setURL(URL_DISCORD)
                .setAuthor("author: " + msg.author.username, POLYGON_LOGO_COLOR, URL_POLYGON)
                .setDescription(BOT_NAME)
