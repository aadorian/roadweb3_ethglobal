...
const InputComponent = () => {
  ...
  const { sendTransaction, state } = useSendTransaction({ transactionName: 'Send NFT' })
    ...
    <div style={{ display: 'flex', flexDirection: 'column' }}>
      <InputRow>
        <Input
          ...
          disabled={disabled}
        />
        <FormTicker>Tokens to:</FormTicker>
        <AddressInput
          id={`AddressInput`}
          type="text"
          value={address}
  ...
  return (
    <ContentBlock style={{ padding: 0 }}>
      <TitleRow>
        ...
        <BalanceWrapper>Your balance: {formatBalance(balance)}</BalanceWrapper>
      </TitleRow>
      <LabelRow>
